...
| `banc-locomotion.mjs` | `node banc-locomotion.mjs` : le **trajet COMMANDÉ, chronométré**. Même noyau que `banc-deambulation` (le vrai `wander.ts` sous node), mais on lui donne des ORDRES au lieu de le laisser vivre : temps **clic → premier pas** pour 30 / 90 / 180° d'écart de cap, temps de trajet complet pour 2 / 5 / 10 / 15 m en ligne droite, trajets **qui tournent** en pièce ouverte, et trajets **à étapes** dans un couloir en L (le seul décor où un itinéraire A\* existe). Aucun tirage aléatoire : les chiffres sont exacts, pas moyennés. Rejouable sur un ancien arbre : `HANAMI_ROOT=<worktree>` |
...
| clip de la famille **Rocketbox** (`rb-…`) | `rb-idle` **et** `rb-idle-talking` | le face à face a deux familles ÉTANCHES (cf. `vrma/README.fr.md`) : un `rb-` n'est jamais fondu vers un socle d'Overte, et le juger contre lui mesurerait la distance entre deux studios — 16,5 à 20,3 cm — pas un défaut. Même erreur que « clip assis contre socle debout » |
...
distinguer** (cf. `vrma/README.fr.md`) :
